    ...
    def viewAppointment(user):
        ...
        while True:
            ...
            while stage == 1:
                ...
                GPNavigator.chooseAppointment(selectedNhsNo, selectedBookingNo)

    def chooseAppointment(selectedNhsNo, selectedBookingNo):
            """
            transaction of the booking confirmation
            ask the user whether they wish to confirm or reject it
            for confirm it will automatically reject all bookings in the same time slot
            ...
            while True:
                ...
                #         stage = 1
                # while stage == 1:
                    print("You selected this patient")
                    print(selectedNhsNo)
                    Qtext = "SELECT visit.BookingNo, visit.Timeslot, visit.NHSNo, users.firstName, users.lastName, visit.Confirmed, users.birthday, users.phoneNo, users.HomeAddress, users.postcode "
                    #Qtext = "SELECT visit.NHSNo "
                    Qtext += "FROM visit INNER JOIN users ON visit.NHSNo = users.ID "
                    Qtext += "WHERE visit.StaffID = ? AND visit.NHSNo = ? "
                    print(Qtext)
                    Qbooked = SQLQuerry(Qtext)
                    # selectedDatePlus1 = selectedDate + datetime.timedelta(days=1)
                    Qbookedresult = Qbooked.executeFetchAll(decrypter=user.encryptionKey, parameters = (user.ID, selectedNhsNo))

                    print(tabulate(Qbookedresult, headers=["BookingNo", "timeslot", "Patient NHSNo",  "P. First Name", "P. Last Name", "Confirmed", "birthday", "phoneNo", "HomeAddress", "postcode"]))

                    userInput = parser.selectionParser(options={"D": "add diagnosis", "N": "add notes", "P": " edit perscription", "--back": "back to previous page"})
                    if userInput == "--back":
                        return
                    elif userInput == "D":
                        # stage = 1
                        # print("Here is your diagnosis")
                        Qtext = "SELECT visit.diagnosis, visit.NHSNo "
                        # Qtext = "SELECT visit.NHSNo "
                        Qtext += "FROM visit "
                        Qtext += "WHERE visit.StaffID = ? AND visit.NHSNo = ? "
                        #print(Qtext)
                        ...
                        Qbookedresult = Qbooked.executeFetchAll(decrypter=user.encryptionKey,parameters=(user.ID, selectedNhsNo))
                        #print(Qbookedresult)
                        print(tabulate(Qbookedresult,headers=["diagnosis", "NHSNo"]))

                        QuerryDiagnosis = SQLQuerry(" UPDATE Visit SET diagnosis = ? WHERE NHSNo = ? ")
                        ...
                        exeQuerryDiagnosis = QuerryDiagnosis.executeCommit((diagnosisInput, selectedNhsNo))
                        print("Your diagnosis has been given, you are going back to the patient info page...")

                    elif userInput == "N":
                        print("Here is your note")
                        Qtext = "SELECT visit.notes, visit.NHSNo "
                        # Qtext = "SELECT visit.NHSNo "
                        Qtext += "FROM visit "
                        Qtext += "WHERE visit.StaffID = ? AND visit.NHSNo = ? "
                        #print(Qtext)
                        ...
                        Qbookedresult = Qbooked.executeFetchAll(decrypter=user.encryptionKey,parameters=(user.ID, selectedNhsNo))
                        #print(Qbookedresult)
                        print(tabulate(Qbookedresult,headers=["notes", "NHSNo"]))

                        QuerryDiagnosis = SQLQuerry(" UPDATE Visit SET notes = ? WHERE NHSNo = ? ")
                        notesInput = input("Please enter your notes:\n")
                        # print(notesInput)
                        exeQuerryNotes = QuerryDiagnosis.executeCommit((notesInput, selectedNhsNo))
                        print("Your notes has been given,you are going back to the patient info page...")

                    elif userInput == "P":
                        print("Here is your perscription")
                        Qtext = "SELECT perscription.BookingNo, perscription.drugName, perscription.quantity, perscription.instructions "
                        # Qtext = "SELECT visit.NHSNo "
                        Qtext += "FROM perscription INNER JOIN visit ON perscription.BookingNo = visit.BookingNo "
                        Qtext += "WHERE visit.StaffID = ? AND visit.BookingNo = ? "
                        # print(Qtext)
                        Qbooked = SQLQuerry(Qtext)
                        print(selectedBookingNo)
                        Qbookedresult = Qbooked.executeFetchAll(decrypter=user.encryptionKey, parameters=(user.ID, selectedBookingNo))

                        # print(Qbookedresult)
                        print(tabulate(Qbookedresult, headers=["BookingNo", "drugName", "quantity", "instructions"]))

                        userInput = parser.selectionParser(options={"A": "add perscription", "R": "remove perscription", "--back": "back to previous page"})
                        if userInput == "--back":
                            return
                        elif userInput == "A":
                            Querryperscription = SQLQuerry(" INSERT INTO perscription (BookingNo, drugName, quantity, instructions) VALUES (?, ?, ?, ?)  ")
                            ...
                            exeQuerryperscription = Querryperscription.executeCommit((BookingNo, drugName, quantity, instructions))
                            print("Your perscription has been given,you are going back to the patient info page...")

                        elif userInput == "R":
                            Querryperscription = SQLQuerry(" DELETE FROM perscription WHERE BookingNo = ?  ")
                            BookingNo = input("Please enter your BookingNo:\n")
                            # print(notesInput)
                            exeQuerryperscription = Querryperscription.executeCommit((BookingNo,))
                            print("this perscription has been removed,you are going back to the patient info page...")
