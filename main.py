...
class MenuHelper:
    ...
    def login():
        for i in range(4, -1, -1):
            ...
            try:
                ...
                username_query = SQLQuery("SELECT username, passCode, Deactivated, UserType FROM Users WHERE username == ?")
        ...
        for i in range(4, -1, -1):
            try:
                # the PW is saved in hash so need to convert to hash to compare special parsing function becasue it will
                # hide the needed
    ...
    def dispatcher(username, user_type):
        if user_type == "GP":
            ...
            gp_user = GP(username)
            gp_user.print_hello()
            gp_user.print_information()
            gp_user.main_menu()
        elif user_type == "Patient":
            from Patient import Patient
            gp_user = Patient(username)
            gp_user.print_hello()
            gp_user.print_information()
            gp_user.main_menu()
